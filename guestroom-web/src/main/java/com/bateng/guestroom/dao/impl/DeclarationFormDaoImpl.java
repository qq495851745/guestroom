...
public class DeclarationFormDaoImpl implements DeclarationFormRepository {
    ...
    public PageVo<DeclarationForm> findDeclarationFormByPage(PageVo<DeclarationForm> pageVo, DeclarationForm declarationForm) {
        ...
        //获取是否有状态
        if (declarationForm.getDeclarationFormStatus() != null && declarationForm.getDeclarationFormStatus().getId() != null && declarationForm.getDeclarationFormStatus().getId() != 0) {
            sb.append(" and df.declarationFormStatus.id = :declarationFormStatusId");
            params.put("declarationFormStatusId", declarationForm.getDeclarationFormStatus().getId());
        }
...
//        sb.append(" select * from ");
    ...
    public List<Object> findAjaxIndex02(RoomOptionVo roomOptionVo) {
        ...
        sb1.append("SELECT t1.roid , t1.roname , count(t2.did) as total1 from t_room_option t1 left outer  join t_declaration_form t2 on (t1.roid = t2.roomOption_id ) where 1=1 and t1.delflag=1 ");
        if(roomOptionVo.getTime01()!=null){
        ...
        sb1.append(" group by t1.roid , t1.roname having count(t2.did)>0 ");


        StringBuffer sb2 = new StringBuffer();
        sb2.append(" select  t1.roomOption_id from t_declaration_form t1 inner join t_repair_form t2 on (t1.did = t2.declaration_form_id) where t2.declaration_form_status_id =5 ");
        if(roomOptionVo.getTime01()!=null){
            sb2.append(" and t1.actualdate >= :time03");
            params.put("time03",roomOptionVo.getTime01());
        }
        if(roomOptionVo.getTime02()!=null){
            sb2.append(" and t1.actualdate <= :time04");
            params.put("time04",roomOptionVo.getTime02());
        }
        sb2.append(" group by t1.roomOption_id ");


        StringBuffer sb3 = new StringBuffer();
        sb3.append(" select t3.roid ,t3.roname,count(t4.roomOption_id) as total2 from t_room_option t3 LEFT OUTER JOIN (");
        sb3.append(sb2.toString());
        sb3.append(" ) as t4 on (t3.roid = t4.roomOption_id) group by t3.roid ,t3.roname");

        StringBuffer sb4 = new StringBuffer();
        sb4.append("select t1.roid,t1.roname,t1.total1,t2.total2 , total2/total1 as re from ( ");
        sb4.append(sb1.toString());
        sb4.append(" ) as t1 inner join ( ");
        sb4.append(sb3.toString());
        sb4.append(" ) t2 on (t1.roid = t2.roid) ");


        sb.append(" select * from ( ");
        sb.append(sb4.toString());
        sb.append(" ) as t order by t.re desc ");
