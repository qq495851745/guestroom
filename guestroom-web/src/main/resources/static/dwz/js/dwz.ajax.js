...
function navTabAjaxDone(json) {
    ...
    if (json[DWZ.keys.statusCode] == DWZ.statusCode.ok) {
        if (json.navTabId) { //把指定navTab页面标记为需要“重新载入”。注意navTabId不能是当前navTab页面的
            ...
            var oldIndex = navTab._currentIndex;
            if (!!json.oldTabId)
                navTab._currentIndex = navTab._indexTabId(json.oldTabId);//刷新其它tab
            var $pagerForm = $("#pagerForm", navTab.getCurrentPanel());
            /* if (!!json.oldTabId)
                 $pagerForm = $("#pagerForm", navTab.getPanel(json.oldTabId));*/
            var args = $pagerForm.size() > 0 ? $pagerForm.serializeArray() : {}
            navTabPageBreak(args, json.rel);
            if (json.refresh) {//刷新树形菜单
                btTree(json);
            }
            navTab._currentIndex = oldIndex;
        }
            ...
            $this.click(function (event) {
                ...
                if (title) {
                    alertMsg.confirm(title, {
                        okCall: function () {
                            ajaxTodo(url, $this.attr("callback"), $this.attr("_method"));
                        }
                    });
                } else {
